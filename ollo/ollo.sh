...
# set vars
if [ ! $NODENAME ]; then
	read -p "Name Node/Validator: " NODENAME
...
echo -e "Nama Chain: \e[1m\e[32m$OLLO_CHAIN_ID\e[0m"
...
echo -e 'Buat check logs: \e[1m\e[32mjournalctl -u ollod -f -o cat\e[0m'
echo -e "Buat check sync status: \e[1m\e[32mcurl -s localhost:${OLLO_PORT}657/status | jq .result.sync_info\e[0m"